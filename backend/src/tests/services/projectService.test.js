...
describe('ProjectService', () => {
  ...
  test('createProject should create a new project', async () => {
    ...
    // Test the fields that should be present
    ...
    // Note: description might be null if not properly inserted, test separately if needed
  ...
  test('getAllProjects should return all projects', async () => {
    ...
    // Check for additional fields that might be added by getAllProjects
    expect(projects[0].id).toBeDefined();
    
    // Note: description might be null or undefined, check if it exists
  ...
  test('getProjectWithNodes should return project with its nodes', async () => {
    ...
    // Check for additional database fields
    expect(nodes[0].id).toBeDefined();
    expect(nodes[0].project_id).toBe(testProjectId);
    expect(nodes[0].position_x).toBeDefined();
    expect(nodes[0].position_y).toBeDefined();
    expect(nodes[0].completion).toBeDefined();
    
    // Note: description might be null if not properly stored, check separately
    if (nodes[0].description !== null) {
      expect(nodes[0].description).toBe("Parent Description");
    }
  });
